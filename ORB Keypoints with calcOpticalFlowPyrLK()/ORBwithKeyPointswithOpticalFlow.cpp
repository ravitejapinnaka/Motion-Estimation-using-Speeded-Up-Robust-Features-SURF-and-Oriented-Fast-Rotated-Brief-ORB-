...
int main( int argc, char* argv[] )
{
    ...
    Ptr<Feature2D> f2d = ORB::create(4); //for ORB, we set nFeatures =  4 to get 4 keypoints
    cv::cvtColor( bgr_frame1, frame_gray1, cv::COLOR_BGR2GRAY);// converting bgr_frame1 to gray image and storing in frame_gray1
    f2d->detect(frame_gray1, keypoints1);// detecting keypoints for first frame using ORB algorithm
    ...
    f2d->detect(frame_gray2, keypoints2);// detecting keypoints for second frame using ORB algorithm
    
    KeyPoint::convert(keypoints2,points2f2,keypointIndexes2);//converting keypoints of 2nd frame to Points2f type
    length = length -1;
    writer.open("/Users/kakarlakeerthy/Desktop/saikkk/output.mp4", CV_FOURCC('M','J','P','G'), fps, size );// output video path and type
    for(;;) // infinite loop
        
    {
        ...
        f2d->detect(frame_gray2, keypoints2);// detecting keypoints for frame_gray2 using ORB algorithm
